 ...
 * 
 * La view devra avoir été créée dans la datasource //TODO: Générer les vues en fonction
 * des datasources et de critères prédéfinis
 * 
 * La fonction {@code filterBy()} sera utilisée sur la requête, 
 * chaque {@link Entry} sera ajouté comme un and à la whereClause
 * 
...
public class ByViewQuery<T extends Entity> implements Query<T> {
	...
	public ByViewQuery(Class<T> resultType, Class<? extends View> view) {
		...
		this.entityPath = new PathBuilder<>(resultType, CaseFormat.UPPER_CAMEL.to(
				CaseFormat.LOWER_CAMEL, resultType.getSimpleName()));
	}
